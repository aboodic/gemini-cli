...
> {
  ...
  /**
   * Checks if a plan path resolves to within the plans directory.
   */
  private isValidPlanPath(planPath: string, plansDir: string): boolean {
    const resolvedPath = path.resolve(this.config.getTargetDir(), planPath);
    return isWithinRoot(resolvedPath, plansDir);
  }
