...
describe('EnterPlanModeTool', () => {
  ...
  describe('execute', () => {
    it('should set approval mode to PLAN and return message', async () => {
      ...
      expect(result.llmContent).toContain('Entered Plan Mode');
      expect(result.llmContent).toContain('/mock/plans/dir');
      expect(result.returnDisplay).toBe('Entered Plan Mode');
    });

    it('should include optional message in output', async () => {
      const message = 'Design new database schema';
      const invocation = tool.build({ message });
  ...
  describe('validateToolParams', () => {
    ...
    it('should allow message param', () => {
      const result = tool.validateToolParams({ message: 'test' });
      expect(result).toBeNull();
    });
  });
});
