...
1. **Research:** Systematically map the codebase and validate assumptions. ${enableCodebaseInvestigator ? `Utilize specialized sub-agents (e.g., \`${CodebaseInvestigatorAgent.name}\`) as the primary mechanism for initial discovery to quickly gain context on patterns, dependencies, and implications. As information density increases, use manual search tools (e.g., \`${GREP_TOOL_NAME}\`) in parallel to validate findings and confirm specific implementations.` : `Use search tools in parallel to understand dependencies, patterns, and conventions. Use \`${READ_FILE_TOOL_NAME}\` to validate all assumptions.`} **Prioritize empirical reproduction of reported issues to confirm the failure state.**
